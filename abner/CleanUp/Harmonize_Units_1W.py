...
from abner.Utilities.Functions_Units import Get_Dict_Conv
...
def Harmonize_Units_1W(PRJ, p_path, pars_input, dso):
    ...
    if True:
        print(f"Checked units in  well: {dso.p_path.name}")
    ...
    for var in var_intersection:
        ...
        if (
            df_varUnitInfo.loc[var, "unit_out"].casefold() in set_unitNames
        ):  # if an alias exists, just rename using the harmonized unit
    ...
    if "NPHI" in dso.df.columns:
        ...
        if nphi_median > 1.0:
            ...
            print(
                f"\tfile {p_path} has a Neutron listed in units of  v/v but it is  PU or %"
            )
