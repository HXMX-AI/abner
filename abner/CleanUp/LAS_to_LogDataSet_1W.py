...
def LAS_to_LogDataSet_1W(PRJ, path_file, pars_input):
    ...
    # VARLIST forming and checking
    varList = [s.mnemonic for s in las.curves]
    varList_dupl = [s for s in varList if ":" in s]
    if len(varList_dupl) > 0:
        print(f"Multiple curves: {varList_dupl}")
        LogDs.duplVars = len(varList_dupl)
    # if 'DEPTH' in varList_dupl:
    #     varList_drop = varList_dupl[1:].copy()
    ...
    # Depth related stuff
    dictUnit, dictUnit_in, depthName_in, depthUnit = Get_Dict_DepthInfo(
