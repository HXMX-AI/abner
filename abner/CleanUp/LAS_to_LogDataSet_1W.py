...
from abner.Utilities.Functions_Units import Get_Dict_DepthInfo
from abner.Utilities.Functions_Misc import BitSize_Convert

# TODO: the typing for the LASFile object needs to be fixed


def LAS_to_LogDataSet_1W(PRJ, path_file, pars_input):

    print(f"{Path(__file__).stem} processing well: {path_file.stem}")
    las: LASFile = lasio.read(path_file)
    LogDs = LogDataSet()
    ...
    LogDs.API = str(las.well.API.value) if "API" in las.well else ""
    LogDs.orgAPI = LogDs.API  # type: ignore
    ...
    if LogDs.API is not None:
        if "-" in str(LogDs.API):
            LogDs.API = str(LogDs.API).replace("-", "")

        if len(str(LogDs.API)) == 10:
            LogDs.API = str(LogDs.API) + "0000"
    ...
    # Depth related stuff
    dictUnit, _, depthName_in, depthUnit = Get_Dict_DepthInfo(
        varList, unitList, PRJ.dict_config
    )
...
if __name__ == "__main__":
    ...
    for i in range(len(fNames)):
        ...
        path_file = Path(pars_input["dir_in"]) / pars_input["fName"]
        newObj = LAS_to_LogDataSet_1W(pars_input, path_file, pars_input)
