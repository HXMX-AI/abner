import  pandas                      as          pd
from    Utilities.Functions_Units   import      Convert_x, Convert_x_to_smpls
...
def Compute_DCAL(PRJ, dso):
    ...
    if False: MakePlot(cali, bs, FL_DCAL, dso.df.DEPTH)
...
def Compute_Pseu_DCAL(PRJ, dso):
    ...
    wLen_Min_conv = Convert_x(wLen_Min, wLen_Min_unit, dso.df_varUnitInfo.loc['DEPTH', 'unit_out'], PRJ)
    wLen_Max_conv = Convert_x(wLen_Max, wLen_Max_unit, dso.df_varUnitInfo.loc['DEPTH', 'unit_out'], PRJ)
    ...
    cali_max       = cali.rolling(nMax, min_periods=nMax, center=True, win_type=None, step=1).max()
    cali_min_step  = cali_max.rolling(nMin, min_periods=1, center=True, win_type=None, step= nMin//2).min()
    ...
    for i in range(len(cali_min_step.index) - 1):
        ...
        cali_min_full.loc[idx_beg:idx_end] = cali_min_step[idx_beg]
    # the last leg
    cali_min_full.loc[idx_end:] = cali_min_step[cali_min_step.index[-1]]

    # TEMP PLOT
    if False: MakePlot(cali, cali_max, cali_min_full, dso.df['DEPTH'][cond], dso.wellName+ '  CaliMinFULl')


    BS_PSEU            = cali_min_full.rolling(nMax, min_periods=1, center=True, win_type=None).min()
    ...
    # Just to PLOT
    if False: MakePlot(cali, BS_PSEU, FL_DCAL_PSEU,  zzzz, dso.wellName)
...
def Compute_Mud_Signal(PRJ, dso):
    ...
    assert dso.df_varUnitInfo.loc['CALI', 'unit_out'] == 'in', print('BoreholeComputations_Mud_Signal CALI not in inches')
    ...
    padLen_conv   = Convert_x(padLen, padLen_unit, 'in', PRJ)
    ...
    cali             = dso.df['CALI'].copy()
    Area             = (vertRes_smpls) *  (doi_conv - standoff_conv)    # not true area, skipping sampRate
    ...
    df_temp.iloc[:padLen_smpls,: ] = (df_temp.iloc[:padLen_smpls,:] - df_temp.iloc[padLen_smpls,:]).copy()
    ...
    unshifted_mud_sgnl   = (df_temp.iloc[padLen_smpls,:] - (vertRes_smpls * standoff_conv) ) / Area

    b                    = list(unshifted_mud_sgnl.loc[padLen_smpls-1:])
    mud_sgnl.loc[j0:j1]    = b
    if False:
        MakePlot(cali, unshifted_mud_sgnl*10, mud_sgnl*10, range(len(cali)))


    if False:            print(f'{max_diff=}')
