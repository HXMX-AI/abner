...
def Casing_Check_1W(PRJ, p_path, pars_input, dso):
    ...
    if "DTCO" not in dso.df_varUnitInfo["var_out"]:
        print(
            f"\tNo Casing Check: well {p_path} does not have DTCO, skipping the process"
        )
        ...
        # CALCULATE
        wLen_smpls = Convert_x_to_smpls(
            dt_casing_thick, dt_casing_thick_unit, PRJ, dso, always_odd=True
        )
        dt = dso.df["DTCO"].copy()
        ...
        dt_roll_min = flag_in.rolling(
            window=wLen_smpls, min_periods=1, center=False
        ).min()
