...
from        abner.Utilities.Functions_Units           import          Get_Units_Recognized, Get_Depth_Variable_Unit
...
def CSV_to_LogDataSet_1W(PRJ, path_file, pars_input):
    ...
    if 'wellName' in csv.columns:
        LogDs.wellName = csv.loc[csv.index[-1],'wellName']
    else:
    ...
    dictUnit                             = dict(zip(LogDs.df_varUnitInfo.index, LogDs.df_varUnitInfo['unit_in']))  #for historical reasons
