...
from    Utilities.Functions_Misc    import      Check_All_Bool
...
def Remove_OOR_1W(PRJ, p_path, pars_input, dso):
    ...
    for var in theCols:
        if var in df_harm.index:
            ...
            cond                = dso.df[var].between(x_left, x_right, inclusive = 'both')   # Negate, want to know bad
    ...
    if False: status = Check_All_Bool(df_flag)      #f or debugging
