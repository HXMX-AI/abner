...
def Depth_Check_1W(PRJ, p_path, pars_input, dso):
    ...
    # To get a positive smpling rate for logging up, periods should be set to 1.
    ...
    if sampRate is None:
        ...
        abs(sampRate_meas - sampRate) > depth_inc_tol
    ):  # Compare to the incoming sampRate
        Say_It(f" Mismatch:  header versus computed sampling rates are different ")
        print(f"{p_path=}")
        print(
            f"well {dso.wellName} resetting sampling rate to {sampRate} to {sampRate_meas}"
        )
    ...
    if cond.sum() > 0:
        ...
        if drop_negative_depth:
            cond = dso.df["DEPTH"] >= 0
            dso.df = dso.depth.loc[cond, :].copy()
    ...
    if null_count > 0:
        ...
        if drop_negative_depth:
            dso.df.dropna(
                axis=0, how="any", subset=["DEPTH"], inplace=True, ignore_index=True
            )
    else:
    ...
    if pars_input["save2pck"]:
        dso.Save2Pck(p_path)
    if True:
        print(f"Checked DEPTH in   well: {dso.wellName}")
...
if __name__ == "__main__":
    ...
    for pckFileName in fileNames:
        ...
        new_dso = Depth_Check_1W(None, pckFileName, pars_input, dso)
