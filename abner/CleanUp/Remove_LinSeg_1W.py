...
def Remove_LinSeg_1W(PRJ, p_path, pars_input, dso):
    ...
    set_varList_def = set(dso.df_varUnitInfo["var_out"]).intersection(
        set(df_harm.index)
    )
    ...
    if padLen_unit != "smpls":
        padLen_smpls = Convert_x_to_smpls(
            padLen, padLen_unit, PRJ, dso, always_odd=True
        )
    else:
        padLen_smpls = padLen
    ...
    df_roll = df_temp_diff.rolling(
        window=wLen_smpls, min_periods=1, center=True, win_type=None, closed=None
    ).var()
