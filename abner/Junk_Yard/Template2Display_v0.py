...
def Template2Display(inputs):
    ...
    props_tracks, props_logs, _, _ = Get_Layout_Properties(inputs)
    df = inputs["df_in"]
    ...
    for n in range(num_tracks):
        ...
        ax = fig.add_axes(coords_n)
        ax.yaxis.set_tick_params(labelsize=yLabelSize)
        ax.xaxis.set_tick_params(labelsize=yLabelSize)
        ax.xaxis.set_tick_params(labelbottom=False)
        ax.grid(axis="both", linestyle="-", c=gridColor)
        axs.append(ax)

        # xticks
        xTicks = np.linspace(
            props_tracks[n]["xL"], props_tracks[n]["xR"], props_tracks[n]["nGrid"] + 1
        )
...
if __name__ == "__main__":
    ...
    gen_defaults = pd.Series(df_defaults.Value, index=df_defaults.Property)
