...
from abner.Visualization.Make_LogPlot import Make_LogPlot
from abner.GUIs.GUI_GetFileName import GUI_GetFileName
from abner.Visualization.Last_LogView_Run import (
    Save_Last_LogView_Run,
    Load_Last_LogView_Run,
)
...
def Display_Logs():
    ...
    def Select_Project():
        ...
        if files_in_dir is None:
            raise ValueError("files_in_dir is None")
    ...
    def Select_Well():
        ...
        prjName.set(
            str(temp.parent)
        )  # MAYBE not set this? But then files and prj may be different
    ...
    def Display_Selected_Well():
        ...
        gen_defaults = pd.Series(
            data=df_defaults.Value.values, index=df_defaults.Property
        )
    ...
    root_new.title("LOG PLOT (log.pck files ONLY)")
    root_new.geometry("800x550")
    ...
    for n in range(len(label_text)):
        temp = tk.Label(root_new, text=label_text[n])
    ...
    e_prjName = tk.Entry(root_new, width=110, textvariable=prjName)
    e_prjName.grid(row=n + 1, column=1, sticky=tk.NSEW, pady=5)

    # wellName
    wellName = tk.StringVar()
    e_wellName = tk.Entry(root_new, width=110, textvariable=wellName)
    e_wellName.grid(row=n + 2, column=1, sticky=tk.NSEW, pady=5)

    # Template
    template_selected = tk.StringVar()
    e_template_selected = tk.Entry(root_new, width=110, textvariable=template_selected)
    e_template_selected.grid(row=n + 3, column=1, sticky=tk.NSEW, pady=5)

    # FRAME 1 ==============================================================================================================
    frame1 = tk.Frame(root_new, width=300, height=350, bg="yellow", relief=tk.RAISED)
    ...
    # Frame 2 ==============================================================================================================
    frame2 = tk.Frame(root_new, width=300, height=300, bg="lightgray", relief=tk.RAISED)
    ...
    display_menu.add_command(
        label="Display Selected Well", command=Display_Selected_Well
    )
