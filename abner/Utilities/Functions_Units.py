...
def Get_Depth_Variable_Unit(dict_config, dictUnit):
    ...
    set_depth_variables        = set(df_variables).intersection(set(depth_variables_recognized))
...
def Get_Dict_Conv(df_unitConv, unit_in, unit_out):
    ...
    if (unit_in not in list(df_temp['unit_in'])) or (unit_out not in list(df_temp['unit_out'])):
        print(f'\tGet_Dict_Conv: either {unit_in} or {unit_out} not defined in configuration tables')
        return {}
    elif unit_in == unit_out:
        if False: print('\tGet_Dict_Conv: units are equal, wasting time!')
        return dict_conv
    else:
        ...
        if conversion.shape != (1,5):
            print(f'\tGet_Dict_Conv: no  conversion defined  for {unit_in} --> {unit_out}')
            return {}
        else:
