...
from    Visualization.Get_Layout_Properties     import      Get_Layout_Properties
from    matplotlib.lines                        import      Line2D
from    Visualization.Add_Slider                import      Add_Slider
...
def Make_LogPlot(inputs):
    ...
    props_tracks, props_logs, header_height, max_header_rows = Get_Layout_Properties(inputs)
    df                                      = inputs['df_in']
    ...
    for t in range(num_tracks):
        ...
        if ( pd.isna( props_logs.loc[xName,'shdTo'] ) == False)  and (xName not in var_missing):
            axs[t].fill_betweenx(depth,x, color=props_logs.loc[xName,'shdClr'], alpha = 1)
    ...
    for t in range(num_tracks):
        ...
        for n in range(num_logs):
            ...
            axs_hdr[t].text( 0.5, ycoord, xName,
                            rotation            = myRotation,
            ...
            if myRotation == 'horizontal':
                ...
                if xL == int(xL): xL = int(xL)
                axs_hdr[t].text( 0.015, ycoord, xL,
            ...
            if myRotation == 'horizontal':
                ...
                if xR == int(xR): xR = int(xR)
                axs_hdr[t].text(0.985, ycoord, xR,
                                horizontalalignment = 'right',
                                verticalalignment   = 'center',
                                color               = props_logs.loc[xName, 'color'],
                                fontsize            = hdrLabelSize)

                line = Line2D([0.0, 1.0], [ycoord-y0/2, ycoord-y0/2], color = props_logs.loc[xName, 'color'],
                                                              linewidth  = props_logs.loc[xName,'width'],
                                                              linestyle  = props_logs.loc[xName,'style'] )
